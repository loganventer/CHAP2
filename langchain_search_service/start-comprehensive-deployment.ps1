...
param(
    ...
    [switch]$ForceRebuild,
    [switch]$ForceGpu,
    [switch]$ApplyAllFixes
)

Write-Host "========================================" -ForegroundColor Green
Write-Host "CHAP2 Comprehensive Deployment" -ForegroundColor Green
Write-Host "========================================" -ForegroundColor Green
...
# Step 3: Apply All Fixes (if requested or if this is a comprehensive deployment)
if ($ApplyAllFixes -or $true) {
    Write-Host "Step 3: Applying comprehensive fixes..." -ForegroundColor Yellow
    
    # Fix 1: Windows Buffering Issues
    Write-Host "   Fix 1: Applying Windows buffering fixes..." -ForegroundColor Gray
    
    # Fix Windows buffering issues in HomeController.cs
    Write-Host "     Applying Windows buffering fixes to HomeController.cs..." -ForegroundColor Gray
...
'@
    ...
    Write-Host "     Applying Windows buffering fixes to Program.cs..." -ForegroundColor Gray
...
'@
    ...
    # Fix 2: API Routes Configuration
    Write-Host "   Fix 2: Updating API routes configuration..." -ForegroundColor Gray
...
"@
    ...
    # Fix 2: Update Docker Compose files with correct API routes
    Write-Host "   Fix 2: Updating Docker Compose files..." -ForegroundColor Gray
    ...
    # Fix 3: Ensure Dockerfiles have curl for connectivity testing
    Write-Host "   Fix 3: Checking Dockerfiles for curl..." -ForegroundColor Gray
    
    # Check if curl is in the main Dockerfile
    $dockerfileContent = Get-Content -Path "Dockerfile" -Raw
    if ($dockerfileContent -notmatch "curl") {
        Write-Host "     WARNING: curl not found in main Dockerfile" -ForegroundColor Yellow
    } else {
        Write-Host "     PASS: curl found in main Dockerfile" -ForegroundColor Green
    }
    
    # Check if curl is in the webportal Dockerfile
    $webportalDockerfileContent = Get-Content -Path "Dockerfile.webportal.simple" -Raw
    if ($webportalDockerfileContent -notmatch "curl") {
        Write-Host "     WARNING: curl not found in webportal Dockerfile" -ForegroundColor Yellow
    } else {
        Write-Host "     PASS: curl found in webportal Dockerfile" -ForegroundColor Green
    }

    Write-Host "   PASS: All fixes applied successfully" -ForegroundColor Green
} else {
    Write-Host "Step 3: Skipping comprehensive fixes (ApplyAllFixes not specified)" -ForegroundColor Yellow
}
...
Write-Host ""
Write-Host "Comprehensive Deployment Summary:" -ForegroundColor Yellow
Write-Host "- All fixes applied automatically" -ForegroundColor White
Write-Host "- GPU detection and configuration completed" -ForegroundColor White
Write-Host "- All services deployed and tested" -ForegroundColor White
...
Write-Host ""
Write-Host "========================================" -ForegroundColor Green
Write-Host "Comprehensive Deployment and cleanup completed!" -ForegroundColor Green
Write-Host "========================================" -ForegroundColor Green 