            ...
            try
            {
                ...
                if (searchString.Length == 1)
                {
                    ...
                    if (IsValidMusicalKey(searchChar))
                    {
                        ...
                        if (keyResults != null)
                        {
                            // Apply sorting: exact key match first, then alphabetically by title
                            var sorted = keyResults
                    ...
                    else
                    {
                        ...
                        if (textResults != null)
                        {
                            // Apply sorting: title match first, then text match, then alphabetically by title
                            var sorted = textResults.OrderByDescending(r =>
                ...
                if (results != null)
                {
                    ...
                    {
                        // Order by exact key match first, then by title alphabetically
                        sorted = results
                            .OrderByDescending(r => r.Key.ToString().Equals(searchString, StringComparison.OrdinalIgnoreCase))
                            .ThenBy(r => r.Name, StringComparer.OrdinalIgnoreCase)
                            .ToList();
                    ...
                    {
                        // For non-key searches, order by title match first, then text match, then alphabetically by title
