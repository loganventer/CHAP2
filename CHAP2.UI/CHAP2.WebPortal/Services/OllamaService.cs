...
public class OllamaService : IOllamaService
{
    ...
    public async IAsyncEnumerable<string> GenerateStreamingResponseWithContextAsync(string prompt, string context)
    {
        var request = new OllamaRequest
        {
            Model = _settings.Model,
        ...
        {
            var errorContent = await response.Content.ReadAsStringAsync();
            _logger.LogError("Failed to generate streaming response with context from Ollama. Status: {StatusCode}, Error: {Error}", 
                response.StatusCode, errorContent);
            throw new Exception($"Failed to generate streaming response with context: {response.StatusCode} - {errorContent}");
        }
